...
func TestAnd(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: &AND{GroupOperator: GroupOperator{items: []Operator{Eq("one", "two")}}},
		},
		{
		...
		},
		{
			...
			want: &AND{GroupOperator: GroupOperator{items: []Operator{
				Eq("one", "two"),
				Eq("three", "four"),
			}}},
		},
...
func TestAND_Where(t *testing.T) {
	tests := []struct {
		name   string
		op     *AND
		truthy bool
		...
		{
			name:   "multiple items",
			op:     And(Eq("one", "two"), Eq("three", "four"), Eq("five", "six")),
			truthy: true,
			want:   "(one = :one AND three = :three AND five = :five)",
		},
		{
			name:   "negate multiple items",
			op:     And(Eq("one", "two"), Eq("three", "four"), Eq("five", "six")),
			truthy: false,
			want:   "NOT (one = :one AND three = :three AND five = :five)",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
